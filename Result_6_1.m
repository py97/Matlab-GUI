...
set(hObject,'numbertitle','off','name','磨损及剩余强度分析');
...
h=msgbox('请稍后，计算中……','提示');
%计算结果------------------------------------------------------------------
%PS:优化时可将W_i7,W_i8,W_i10,W_i11 合并
...
%均匀磨损极限剩余抗拉强度（MPa）  -42.11*(W_i,12)^2+1015.29*(W_i,12)-2802.67
W_i15 = {};
for i = 1:size(U_i1,1)
...
pass_Data{13} = result_Data;    %返回结果数据
...
if size(get(handles.y_6uitable2,'Data'),1)==0
    ...
    if (Filename==0 & Pathname==0)
            ...
            DTI6_1 = Document.Tables.Item(1); % 或DTI = Tables;
            ...
            DTI6_2 = Document.Tables.Item(2); % 或DTI = Tables;
            ...
            %导出图片----------------------------------------------------------------------------------------------------------------------
             Selection.Start = Content.end; % 设定光标位置为最后
                ...
                Selection.Start = Content.end; % 设定光标位置为最后
                Selection.TypeParagraph;    %插入新的空段落       
                hgexport(hObject_fig2, '-clipboard');    %将图形复制到Microsoft? Windows粘贴板
